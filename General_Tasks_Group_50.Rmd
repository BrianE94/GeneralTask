...
If-function is necessary, otherwise error messages would pop up. The "dplyr" and "fitdistrplus" packages are in conflict regarding the "select" function. To avoid emerging error messages, we further specified the "dplyr" "select" function. 
...
Thus, it is necessary to find out which distribution model fits the data best. For this purpose, a Cullen and Frey graph is plotted. For reference, check this [paper](https://cran.r-project.org/web/packages/fitdistrplus/vignettes/paper2JSS.pdf).
...
fln <- fitdist(verzugstage, "lognormal")

denscomp(list(fw, fg, fln), legendtext=plot.legend) # Density plot
qqcomp(list(fw, fg, fln), legendtext=plot.legend) # Q-Q plot
cdfcomp(list(fw, fg, fln), legendtext=plot.legend) # Empirical and theoretical cumulative density function
ppcomp(list(fw, fg, fln), legendtext=plot.legend) # P-P plot
```
