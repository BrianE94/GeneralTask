...
In order to find the parts T4 which ended up in vehicles registered in the city of Dortmund, we filtered the cars according to this municipality. 
Then we linked all components that have been installed in vehicles together in one table. Furthermore, we joined both tables combining all registered vehicles in Dortmund with the engine component. Finally, we filtered for all engines K1BE1, which all include the T4 part, that are also assembled in vehicles registered in Dortmund. 
