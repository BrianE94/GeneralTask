...
The histogram suggests a normal distribution. Q-Q plots are used to verify this assumption. For reference, check [this] (https://data.library.virginia.edu/understanding-q-q-plots/#). 
...
The normal Q-Q plot shows that the points are not on a straight line. 
Therefore, both sets of quantiles are not normal distributed.

It is necessary to find out which distribution model fits the data best. 
...
"X1" = `numeric`, "IDNummer" = `character`, "Gemeinden" = `character` and "Zulassung" = `date`. 
