...
Histogram visualization according to [plotly-r.com](https://plotly-r.com/overview.html#intro-plotly)

```{r, message=FALSE, warning=FALSE}

# Histogram of logistics delay 
fig1 <- logistics_delay%>%
  plot_ly(x = ~Logistikverzug, alpha = 0.8, nbinsx = 30) %>%
  add_histogram()%>%
  ...
  add_text(
    ...
    cliponaxis = FALSE
    #title = "Histogram of logistics delay"
  )%>%
  hide_legend()
...
# Density
#density_logistics_delay <- Density(logistics_delay$Logistikverzug)

#glimpse(density_logistics_delay)

#fig2 <- plot_ly(x = ~density_logistics_delay, type = "scatter", mode = "lines", fill = 'tozeroy')%>%
 # layout(xaxis = list(title = "abc"), yaxis = list(title = "density"))

#fig2

#density_f <- density(logistics_delay, kernel = 'gaussian', bw = 0.1)

#fig2 <- plot_ly()%>%
#  add_lines(x = density_f$x, y = density_f$y, line = list(color = 'olivedrab'))
