...
To understand the data distribution of logistics delay, we visualized the data by plotting a histogram.

```{r, fig.align="center"}
hist(verzugstage, breaks=10, main="Histogram of Logistics Delay", xlab="Logistics delay in days")
```

The histogram suggests a normal distribution. Q-Q plots are used to visually verify this assumption, while the Shapiro-Wilk normality test is used for the mathematical testing. For reference, check [University of Virginia](https://data.library.virginia.edu/understanding-q-q-plots/#), the specific [R Manual](https://stat.ethz.ch/R-manual/R-devel/library/stats/html/shapiro.test.html) and [TU Dresden](https://wwwpub.zih.tu-dresden.de/~petzoldt/elements_de.pdf).
...
The normal Q-Q plot shows that the points are not on a straight line. Therefore, both sets of quantiles are not normally distributed. 

```{r}
# Mathematical testing of normal distribution with the Shapiro-Wilk normality test
shapiro.test(sample(verzugstage, 5000))
```

Furthermore, we conducted the Shapiro-Wilk normality test with a random sample of size 5000 of the logistics delay vector "verzugstage" (this is due to the limitation of the R function). With p=2.2e-16 (almost zero percent) probability we would be wrong that it is **not** a normal distribution.
...
```{r, fig.align="center"}
# Cullen and Frey graph: Skewness-kurtosis plot
...
# Empirical and theoretical probability P-P plot
