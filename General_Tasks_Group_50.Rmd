...
The "dplyr" and "fitdistrplus" packages are in conflict regarding the "select" function. To avoid emerging error messages, we further specified the "dplyr" "select" function. 
...
install_load("readr", "magrittr", "dplyr", "plotly", "fitdistrplus")
...
All necessary CSV files are imported. Note that the first column of most CSV files needs to be removed to get uniform datasets. 
...
```{r, message=FALSE, warning=FALSE}
# Creating new data set with calculated logistics delay
...
# Storing calculated logistics delay in a separate variable
verzugstage <- logistics_delay$Logistikverzug

# Understanding structure of new variables
...
By visualizing the data of logistics delay by means of plotting a histogram, we get a better understanding of its distribution.

```{r, fig.align="center"}
# Histogram of logistics delay
hist(verzugstage, breaks=10, main="Histogram of Logistics Delay", xlab="Logistics Delay (days)")
```

The histogram suggests a normal distribution. Q-Q plots are used to visually verify this assumption, while the Shapiro-Wilk normality test is used for the mathematical verification. For reference, check [University of Virginia](https://data.library.virginia.edu/understanding-q-q-plots/#), the specific [R Manual](https://stat.ethz.ch/R-manual/R-devel/library/stats/html/shapiro.test.html) and [TU Dresden](https://wwwpub.zih.tu-dresden.de/~petzoldt/elements_de.pdf).

```{r, fig.align="center"}
# Visual testing of normal distribution with Q-Q plots
qqnorm(verzugstage)  
qqline(verzugstage, col="red")
```

The normal Q-Q plot shows that the data are not on a straight line (see red line). Therefore, both sets of quantiles are not normally distributed. 

```{r}
# Mathematical verification of normal distribution with Shapiro-Wilk normality test
shapiro.test(sample(verzugstage, 5000))
```

Furthermore, we conducted the Shapiro-Wilk normality test with a random sample of size 5000 from the logistics delay vector "verzugstage" (this is due to the limitation of the R function). With p=2.2e-16 (almost zero percent) probability we would be wrong that it is **not** a normal distribution.

Thus, it is necessary to find out which distribution model fits the data best. For this purpose, a Cullen and Frey graph is plotted. For reference, check this [research paper](https://cran.r-project.org/web/packages/fitdistrplus/vignettes/paper2JSS.pdf) by Delignette-Muller, M. et al. (2020).

```{r, fig.align="center"}
# Plotting Cullen and Frey graph: a skewness-kurtosis plot
descdist(verzugstage)
```

The skewness-kurtosis plot suggests a lognormal, gamma or Weibull distribution. Following the research paper linked above, four goodness-of-fit plots for the three mentioned distributions are plotted provided by functions "denscomp", "qqcomp", "cdfcomp" and "ppcomp". 

```{r, fig.align="center"}
# Fit of distributions by maximum likelihood estimation
fw <- fitdist(verzugstage, "weibull")
fg <- fitdist(verzugstage, "gamma")
fln <- fitdist(verzugstage, "lnorm")

# Density plot
denscomp(list(fw, fg, fln), legendtext=c("Weibull", "Gamma", "Lognormal"))
# Q-Q plot
qqcomp(list(fw, fg, fln), legendtext=c("Weibull", "Gamma", "Lognormal")) 
# Empirical and theoretical cumulative density function (CDF plot)
cdfcomp(list(fw, fg, fln), legendtext=c("Weibull", "Gamma", "Lognormal")) 
# Empirical and theoretical probabilities P-P plot
ppcomp(list(fw, fg, fln), legendtext=c("Weibull", "Gamma", "Lognormal")) 
```

The density plot and CDF plot suggest that the Lognormal distribution function is more likely to fit the data distribution of logistics delay than the Gamma or Weibull distribution, since both plots are considered to be the classical goodness-of-fit plots (Delignette-Muller, M. et al, 2020). 

The Q-Q plot and P-P plot emphasize the lack-of-fit at the distribution tail respectively center, which means Weibull and Gamma distribution functions are less considered to fit the data distribution of logistics delay (Delignette-Muller, M. et al, 2020).

### Subtask b
**What is the minimum/maximum time between delivering and receiving goods?**

```{r}
# Calculating the minimum/maximum time between delivering and receiving goods
...
```{r}
# Calculating the rounded mean of logistics delay
...
Histogram visualization according to [plotly-r.com](https://plotly-r.com/overview.html#intro-plotly). Density plot visualization according to [Data Camp](https://learn.datacamp.com/courses/interactive-data-visualization-with-plotly-in-r).

```{r, fig.align="center", message=FALSE, warning=FALSE}
# Histogram of logistics delay 
fig1 <- logistics_delay%>%
  plot_ly(x = ~Logistikverzug, alpha = 0.7, nbinsx = 30) %>%
  add_histogram()%>%
  group_by(Logistikverzug)%>%
  summarize(n = n())%>%
  layout(title="Histogram of Logistics Delay", 
    xaxis=list(title="Logistics Delay (days)"), 
...
# Density function of logistics delay
fit <- density(verzugstage, bw=0.5)

fig2 <- plot_ly()%>%
  add_lines(x=~fit$x, y=~fit$y, color="orange", fill="tozeroy")%>%
  layout(title="Density Function of Logistics Delay", xaxis=list(title="Logistics Delay (days)"), yaxis=list(title="Density"))
...
```{r}
# Filtering for all the vehicles registered in Dortmund
zulassungen_dortmund <- zulassungen_alle_fahrzeuge %>%
  filter(Gemeinden=="DORTMUND")

# Joining all car components in one table and selecting the car ID number "ID_Fahrzeug" within all  produced vehicles and the "ID_Motor" because T4 is part of the engine
...
# Comparing all registered vehicles in Dortmund with the component engine list
...
Furthermore, external access from other users are not possible. No application can be run on other computers.
...
To get the specific engine ID number, we filtered for it within the list of all vehicle components. Then, comparing this specific engine ID with the vehicle registration list resulted in the exact location of the stolen car. 
